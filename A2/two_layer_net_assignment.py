...
def nn_forward_backward(
    ...
    correct_prob = - torch.log(prob[range(N),y])
    ...
    grads["b2"] = torch.sum(dscores, dim=0) # it is [C,]
    grads["W2"] = h1.t().mm(dscores) + 2 * reg * W2 # it is H*N * N*C
    
    hidden = dscores.mm(W2.t())
    hidden[h1 == 0] = 0 # ReLU
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    y_pred_score, _ = nn_forward_pass(params, X)
    _, y_pred =  y_pred_score.max(dim = 1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
