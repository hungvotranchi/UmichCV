...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    y_pred = torch.argmax(X.mm(W), axis = 1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def test_one_param_set(
    ...
    # Replace "pass" statement with your code
    cls.train(data_dict["X_train"],data_dict["y_train"], learning_rate= lr, \
                reg = reg, num_iters= num_iters)
    y_pred_test = cls.predict(data_dict["X_train"])
    train_acc = (data_dict["y_train"] == y_pred_test).double().mean().item() * 100
    y_val_test = cls.predict(data_dict["X_val"])
    val_acc = (data_dict["y_val"] == y_val_test).double().mean().item() * 100
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      scores_sample = W.t().mv(X[i])
      scores_sample = scores_sample - scores_sample.max()
      correct_class_score = scores_sample[y[i]]
      loss += torch.log(torch.exp(scores_sample).sum()) - correct_class_score
      for j in range(num_classes):
        if j == y[i]:
          dW[:, j] += torch.exp(scores_sample[j]) / torch.exp(scores_sample).sum() * X[i,:] - X[i,: ]
        else:
          dW[:, j] += torch.exp(scores_sample[j]) / torch.exp(scores_sample).sum() * X[i,:]
...
def softmax_loss_vectorized(
    ...
    scores = X.mm(W)
    scores = scores - scores.max().view(-1, 1)
    correct_scores = scores[range(num_train), y]
    exp_score = torch.exp(scores)
    prob = exp_score / torch.sum(exp_score, dim = 1, keepdim=True)
    correct_prob = - torch.log(prob[range(num_train),y])
    loss = torch.sum(correct_prob)/num_train
    loss += reg * torch.sum(W * W)
...
def softmax_get_search_params():
    ...
    # Replace "pass" statement with your code
    import numpy as np
    learning_rates = np.linspace(1e-2, 2e-1, 4) # [1e-2, 5e-1] 
    # regularization strengths candidates
    regularization_strengths = np.linspace(1e-4, 1e-3, 4)
