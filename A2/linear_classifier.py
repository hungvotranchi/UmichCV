...
def svm_loss_vectorized(
    ...
    pred_score = X.mm(W)
    correct_class_score = pred_score[range(num_element),y].reshape(-1, 1)
    margin = torch.maximum(torch.zeros(num_classes).to("cuda"),pred_score - correct_class_score + 1)
    margin[range(num_element),y] = 0
    loss = margin.sum()/ num_element
    loss += reg * torch.sum(W * W)
    ...
    margin[torch.arange(num_element), y] -= valid_margin
    dW = X.T.mm(margin) / num_element
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def test_one_param_set(
    ...
    y_pred = cls.predict(data_dict["X_train"])
    train_acc = (y_pred == data_dict["y_train"]).double().mean().item() * 100


    y_pred_val = cls.predict(data_dict["X_val"])
    val_acc = (y_pred_val == data_dict["y_val"]).double().mean().item() * 100
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
