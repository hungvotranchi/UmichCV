...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    flat = x_train.shape[1] * x_train.shape[2] * x_train.shape[3]
    for i in range(num_train):
      temp = x_test.view(-1, flat) - x_train[i].view(-1, flat)
      dists[i,:] = torch.sqrt(torch.sum(temp ** 2, dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #distance = train ^ 2 + test ^ 2 - 2.train.test
    mulmat = -2 * train_flat.mm(test_flat.T)
    dists = torch.sum(train_flat ** 2, dim = 1).reshape(-1, 1) +\
            torch.sum(test_flat ** 2, dim = 1) +\
            mulmat
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      _, index = torch.topk(dists[:,i], k, largest=False)
      label = y_train[index]
      label_count = torch.bincount(label)
      y_pred[i] = torch.argmax(label_count)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for fold in range(num_folds):
        x_train_local = [x for x in x_train_folds]
        y_train_local = [y for y in y_train_folds]
        x_test_local = x_train_folds[fold]
        y_test_local = y_train_folds[fold]
        del x_train_local[fold]
        del y_train_local[fold]
        x_train_combine = torch.cat(x_train_local, dim = 0)
        y_train_combine = torch.cat(y_train_local, dim = 0)
        classifier = KnnClassifier(x_train_combine, y_train_combine)
        list_of_acc.append(classifier.check_accuracy(x_test_local, y_test_local, k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean_dict = {key: sum(value)/len(value) for key, value in k_to_accuracies.items()}

    best_k = max(mean_dict, key=mean_dict.get)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
