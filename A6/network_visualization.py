...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
      ...
      grad_norm = grad / (grad.norm(p=2) + 1e-10)

      X_adv.data += learning_rate * grad_norm
      X_adv.grad.zero_()

      if y_pred.argmax().item() == target_y:
        if verbose:
            print(f"Attack succeeded in {i+1} iterations.")
        break
      elif verbose:
        print(f"Iteration {i+1}: Target score = {target_score.item():.4f}")
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    img.grad.data.zero_() # clear the gradient
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
    return img
