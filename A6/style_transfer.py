...
def content_loss(content_weight, content_current, content_original):
    ...
    loss = content_weight * torch.mean(torch.sum((content_current - content_original) ** 2))
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer in enumerate(style_layers):
      style_loss+= style_weights[idx] * torch.sum((gram_matrix(feats[layer]) - style_targets[idx]) ** 2)
    return style_loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def tv_loss(img, tv_weight):
    ...
    # Compute the total variation loss by summing the differences
    loss = tv_weight * (diff_h + diff_w)
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    style_loss = 0
    for idx, layer in enumerate(style_layers):
      style_loss+= style_weights[idx] * torch.sum((guided_gram_matrix(feats[layer], content_masks[layer]) - style_targets[idx]) ** 2)
    return style_loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
