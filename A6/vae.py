...
def reparametrize(mu, logvar):
    ...
    std = torch.exp(0.5 * logvar)  # Shape (N, Z)

    # Reparameterization trick
    z = mu + std * epsilon  # Shape (N, Z)
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    loss_term = nn.functional.binary_cross_entropy(x_hat, x, reduction="sum")
    dl_term = -0.5 * torch.sum(1 + logvar - mu.pow(2) - variance)
    loss = loss_term + dl_term
    loss/=x_hat.shape[0]
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
