...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    hidden_linear = prev_h.mm(Wh) + x.mm(Wx) + b
    next_h = torch.tanh(hidden_linear)
    cache = x, prev_h, Wx, Wh, b, hidden_linear
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x, prev_h, Wx, Wh, b, hidden_linear = cache
    dtanh = (1 - torch.tanh(hidden_linear)**2) * dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for step in range(x.shape[1]):
      next_h, cache_step = rnn_step_forward(x[:,step,:], prev_h, Wx, Wh, b)
      h[:, step, :] = next_h
...
def rnn_backward(dh, cache):
    ...
    print(f"First cache element type: {type(cache[0])}")
    print(f"First cache element length (if it's a tuple or list): {len(cache[0]) if isinstance(cache[0], (tuple, list)) else 'Not a sequence'}")
    
    # Assuming cache[0] is the cache for the first timestep
    if isinstance(cache[0], (tuple, list)):
        # If cache[0] is a sequence, we'll assume it contains x, h, and other elements
        x = cache[0][0]
    else:
        # If cache[0] is not a sequence, it might be a dictionary or some other structure
        # We'll need to adjust based on the actual structure
        raise ValueError("Unexpected cache structure. Please provide more information about the cache contents.")
    ...
    # Initialize the gradient of the next hidden state
    dnext_h = torch.zeros((N, H), device=dh.device, dtype=dh.dtype)
    
    # Iterate backwards through time
    for t in reversed(range(T)):
        # Add the upstream gradient at this timestep to dnext_h
        dnext_h += dh[:, t, :]
        
        # Get the cache for this timestep
        step_cache = cache[t]
        
        # Compute gradients for this timestep
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, step_cache)
        ...
        # Update dnext_h for the next iteration
        dnext_h = dprev_h
    
    # The gradient of the initial hidden state is the final dprev_h
    dh0 = dprev_h
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if self.cell_type == "attn":
            self.conv = nn.Conv2d(self.image_encoder_layer.out_channels, hidden_dim, 1, 1) # (N, H, 4, 4)
        else:
            self.avg_pool_layer = nn.AvgPool2d(4, 4) # (N, C)
            self.transform_layer = nn.Linear(self.image_encoder_layer.out_channels, hidden_dim) # (N, H)

        self.word_embedding_layer = WordEmbedding(vocab_size, wordvec_dim)
        
        if self.cell_type == "rnn":
            self.rnn_layer = RNN(wordvec_dim, hidden_dim)
        elif self.cell_type == "lstm":
            self.lstm_layer = LSTM(wordvec_dim, hidden_dim)
        elif self.cell_type == "attn":
            self.attn_lstm_layer = AttentionLSTM(wordvec_dim, hidden_dim)
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            ...
            images = self.avg_pool_layer(images).squeeze()
            images = self.transform_layer(images)

        captions_in = self.word_embedding_layer(captions_in)
        # print(captions_in.shape)

        # h0 is the transformed image feature
        if self.cell_type == "rnn":
          hidden_states = self.rnn_layer(captions_in, images)
        elif self.cell_type == "lstm":
          hidden_states = self.lstm_layer(captions_in, images)
        elif self.cell_type == "attn":
          hidden_states = self.attn_lstm_layer(captions_in, images)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
          images = self.conv(images) # (N, H, 4, 4)
          prev_h = images.mean(dim = (2, 3)) # (N, H)
          prev_c = images.mean(dim = (2, 3)) # (N, H)

        else:
          images = self.avg_pool_layer(images).squeeze()
          prev_h = self.transform_layer(images)

        x = self.word_embedding_layer([self._start for x in range(N)])
        ...
        for t in range(max_length):
            if self.cell_type == "rnn":
              prev_h = self.rnn_layer.step_forward(x, prev_h)

            elif self.cell_type == "lstm":
              prev_h, prev_c = self.lstm_layer.step_forward(x, prev_h, prev_c)

            elif self.cell_type == "attn":
              attn, attn_weights = dot_product_attention(prev_h, images)
              attn_weights_all[:, t] = attn_weights
              prev_h, prev_c = self.attn_lstm_layer.step_forward(x, prev_h, prev_c, attn)

            scores = self.linear_layer(prev_h)
            _, idx = scores.max(dim = 1)
            x = self.word_embedding_layer(idx)
            captions[:, t] = idx
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your codede
        H = prev_h.shape[1]
        a = x.mm(self.Wx) + prev_h.mm(self.Wh) + self.b # (N, 4H)
        # print(a.shape)
        ai, af, ao, ag = torch.split(a, [H, H, H, H], dim = 1)
        i = torch.sigmoid(ai)
        f = torch.sigmoid(af)
        o = torch.sigmoid(ao)
        g = torch.tanh(ag)
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    flatten_A = torch.flatten(A, start_dim=2)
    # print(flatten_A.shape)
    # print(prev_h.shape)
    weights = torch.matmul(flatten_A.transpose(1, 2), prev_h.unsqueeze(dim=2)).squeeze() / math.sqrt(H) # (N, 16)
    # print(weights.shape)
    normalized_weights = F.softmax(weights, dim = 1)
    # print(normalized_weights.shape)
    attn_weights = normalized_weights.reshape(-1, 4, 4)
    attn = torch.bmm(flatten_A, normalized_weights.unsqueeze(dim=2)).squeeze()
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        H = prev_h.shape[1]
        a = x.mm(self.Wx) + prev_h.mm(self.Wh) + attn.mm(self.Wattn) + self.b # (N, 4H)
        # print(a.shape)
        ai, af, ao, ag = torch.split(a, [H, H, H, H], dim = 1)
        i = torch.sigmoid(ai)
        f = torch.sigmoid(af)
