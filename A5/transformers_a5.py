...
def generate_token_dict(vocab):
    ...
    token_dict = {
      vocab[i]: i for i in range(0, len(vocab))
    }
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def prepocess_input_sequence(
    ...
    out = [token_dict[value] if value in spc_tokens else list(value) for value in split_str]
    out = [int(item) for sublist in out for item in (sublist if isinstance(sublist, list) else [sublist])]
    print(out)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_single(
    ...
    for q in query:
      dot_products = []
      for k in key:
        dot_product = (q @ k) / torch.sqrt(torch.tensor(M))
        dot_products.append(dot_product)
      dot_prod_k.append(dot_products)
    dot_prod_k = torch.tensor(dot_prod_k)
    dot_prod_k = torch.nn.functional.softmax(dot_prod_k, dim = 1)
    out = dot_prod_k.mm(value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    scaling_factor = torch.sqrt(torch.tensor(M, dtype=torch.float32))

    for i in range(N):
      dot_product_batch = torch.bmm(query[i].unsqueeze(0),\
                  key[i].transpose(0, 1).unsqueeze(0)) / scaling_factor
      dot_product_batch = dot_product_batch.squeeze(0)
      for k in range(K):
        weight = torch.softmax(dot_product_batch[k], dim = -1)
        out[i, k] = torch.matmul(weight, value[i])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    # Compute scaled dot product between queries and keys
    attention_scores = torch.bmm(query, key.transpose(1, 2)) / scaling_factor
    ...
    if mask is not None:
      ...
      device_a = attention_scores.device
      attention_scores = attention_scores.masked_fill(mask.to(device_a), float('-inf'))

    # Apply softmax to get the attention weights
    weights_softmax = torch.nn.functional.softmax(attention_scores, dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # Uniform distribution bounded by [-c,c]
        cq = (6/(dim_in + dim_q))**0.5
        torch.nn.init.uniform_(self.q.weight, a=-cq, b=cq)
        ck = (6/(dim_in + dim_q))**0.5
        torch.nn.init.uniform_(self.k.weight, a=-ck, b=ck)
        cv = (6/(dim_in + dim_v))**0.5
        torch.nn.init.uniform_(self.v.weight, a=-cv, b=cv)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.attentions = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for i in range(num_heads)])
        self.decoder = nn.Linear(num_heads * dim_out, dim_in)
        cd = (6/(num_heads*dim_out + dim_in))**0.5
        torch.nn.init.uniform_(self.decoder.weight, a=-cd, b=cd)
        torch.nn.init.constant_(self.decoder.bias, 0)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for i in range(len(self.attentions)):
            y_ = self.attentions[i](query, key, value, mask)  
            y.append(y_)
        # Now: each y_ (2,4,8)
        y = torch.cat(y, dim=2) # (2, 4, 16)
        y  = self.decoder(y)    # (2, 4, 8)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        if len(x.shape)==3:
            N, K, M = x.shape
            mean = torch.mean(x, dim=-1).reshape(N,K,1).repeat(1,1, x.shape[2])
            std = (torch.sum( (x - mean)**2 , dim=2, keepdim=True) / M) **0.5      
        else:
            # (N, K)
            N, K = x.shape
            mean = torch.mean(x, dim=-1).reshape(-1,1).repeat(1,x.shape[1])
            std = (torch.sum( (x - mean)**2 , dim=1, keepdim=True) / K) **0.5
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        cq = (6/(inp_dim + hidden_dim_feedforward))**0.5
        torch.nn.init.uniform_(self.linear_1.weight, a=-cq, b=cq)
        torch.nn.init.uniform_(self.linear_2.weight, a=-cq, b=cq)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.layerNorm_1 = LayerNormalization(emb_dim)
        self.layerNorm_2 = LayerNormalization(emb_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out_att = self.multiheadattn(x, x, x)
        out_ln1 = self.dropout(self.layerNorm_1(out_att + x))
        out_ff = self.feedforward(out_ln1)
        y = self.dropout(self.layerNorm_1(out_ff + out_ln1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    N, K = seq.shape
    mask = torch.ones((N, K, K), dtype=torch.bool)
    for batch in range(N):
      for seq_k in range(K):
        mask[batch,seq_k, :seq_k+1] = False
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out_selfattn = self.attention_self(dec_inp, dec_inp, dec_inp, mask)
        out_norm1 = self.dropout(self.norm1(out_selfattn + dec_inp))
        out_crossattn = self.attention_cross(out_norm1, enc_inp, enc_inp)
        out_norm2 = self.dropout(self.norm2(out_crossattn + out_norm1))
        out_ff = self.feed_forward(out_norm2)
        out_norm3 = self.dropout(self.norm3(out_ff + out_norm2))
        y = self.dropout(out_norm3)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.arange(K) / K
    y = y.unsqueeze(dim = - 1).repeat(1, M).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
