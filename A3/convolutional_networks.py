...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        p2d = (p,p,p,p)
        padded = torch.nn.functional.pad(x, p2d, "constant", 0)
        ...
        H_ = int(1 + (H + 2*p - HH)/s) 
        W_ = int(1 + (W + 2*p - WW)/s)
        out = torch.empty((N, F, H_, W_), dtype=torch.float64).to(x.device)
        ...
        for n in range(N):
          for f in range(F):
            for i in range(0, H_):
              for j in range(0, W_):
                out[n][f][i][j] = torch.sum(w[f, :, :, :]*padded[n, :, i*s:i*s+HH, j*s:j*s+WW])+b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        acum_dw = torch.zeros_like(w)
        acum_dpadded = torch.zeros_like(padded)
        acum_db = torch.zeros_like(b) 
        for n in range(N):
          for f in range(F):
            acum_db[f] += torch.sum(dout[n][f])
            for i in range(0, H_):
              for j in range(0, W_):
                acum_dw[f] += padded[n, :, i*s:i*s+HH, j*s:j*s+WW] * dout[n, f, i, j]
                acum_dpadded[n, :, i*s:i*s+HH, j*s:j*s+WW] += w[f, :, :, :]* dout[n, f, i, j]
        dw = acum_dw 
        dx = acum_dpadded[:,:,1:-1,1:-1]
        db = acum_db 
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros((N, C, H_, W_)).to(x.device)
        for n in range(N):
          for c in range(C):
            for i in range(0, H_):
              for j in range(0, W_):
                out[n, c, i, j] = torch.max(x[n, c, i*s:i*s+ph, j*s:j*s+pw])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        out_CNN_H = int(1 + (H + 2 * conv_param['pad'] - HH) / conv_param['stride'])
        out_CNN_W = int(1 + (W + 2 * conv_param['pad'] - WW) / conv_param['stride'])

        H_out = int(1 + (out_CNN_H - pool_param['pool_height']) / pool_param['stride'])
        W_out = int(1 + (out_CNN_W - pool_param['pool_width']) / pool_param['stride'])
        self.params["W1"] = weight_scale * torch.randn(num_filters,C, filter_size, filter_size, dtype=dtype, \
                                              device= device)
        self.params["b1"] = torch.zeros(num_filters, dtype = dtype, device = device)


        self.params["W2"] = weight_scale * torch.randn(num_filters * H_out * W_out, \
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out_Conv, cache_Conv = Conv_ReLU_Pool.forward(X, self.params["W1"], self.params["b1"], \
                                                  conv_param= conv_param, pool_param=pool_param)
        out_L1, cache_L1 = Linear_ReLU.forward(out_Conv, self.params["W2"], self.params["b2"])   
        scores, cache_L2 = Linear.forward(out_L1, self.params["W3"], self.params["b3"])                                    
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dx, dw, db = Linear.backward(dout, cache_L2)
        grads["W3"] = dw + 2 * self.reg * self.params["W3"]
        grads["b3"] = db

        dx, dw, db = Linear_ReLU.backward(dx, cache_L1)
        grads["W2"] = dw + 2 * self.reg * self.params["W2"]
        grads["b2"] = db

        dx, dw, db = Conv_ReLU_Pool.backward(dx, cache_Conv)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                        ...
                        max_pools=[0, 1],
                    weight_scale=7e-2,
                    reg=1e-5, dtype=torch.float, device= "cuda")
