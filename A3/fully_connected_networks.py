...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out_L1, cache_L1 = Linear_ReLU.forward(X,self.params["W1"], self.params["b1"])
        scores, cache_L2 = Linear.forward(out_L1,self.params["W2"], self.params["b2"])
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers - 1):
          self.params[f"W{i + 1}"] = weight_scale * torch.randn(last_dim, hidden_dims[i], \
                      dtype = dtype, device = device)
          self.params[f"b{i + 1}"] = torch.zeros(hidden_dims[i], device = device, dtype=dtype)
          last_dim = hidden_dims[i]
          
        self.params[f"W{self.num_layers}"] = weight_scale * torch.randn(hidden_dims[self.num_layers - 2],\
                      num_classes, \
                      dtype = dtype, device = device)
    ...
    def loss(self, X, y=None):
        ...
        cache_dict = {}
        for i in range(self.num_layers - 1):
          prev_score, cache_dict[f"cache_LR{i + 1}"] = Linear_ReLU.forward(prev_score,\
          self.params[f"W{i + 1}"], self.params[f"b{i + 1}"])

          
          if self.use_dropout:
            prev_score, cache_dict[f"cache_Dropout{i + 1}"] = Dropout.forward(prev_score, self.dropout_param)
        scores, cache_dict[f"cache_LR{self.num_layers}"] = Linear.forward(prev_score,self.params[f"W{self.num_layers}"], self.params[f"b{self.num_layers}"])
        ...
        last_dout, dw, db  = Linear.backward(dout, cache_dict[f"cache_LR{self.num_layers}"])
        grads[f"W{self.num_layers}"] = dw + 2 * self.params[f"W{self.num_layers}"] * self.reg
        grads[f"b{self.num_layers}"] = db
        for i in range(self.num_layers-1)[::-1]:
          if self.use_dropout:
            last_dout =  Dropout.backward(last_dout, cache_dict[f'cache_Dropout{i + 1}'])
          last_dout, dw, db  = Linear_ReLU.backward(last_dout, cache_dict[f'cache_LR{i + 1}'])
          grads[f'W{i + 1}'] = dw + 2*self.params[f'W{i + 1}']*self.reg
          grads[f'b{i + 1}'] = db
          loss += (self.params[f'W{i + 1}']*self.params[f'W{i + 1}']).sum()*self.reg
...
def get_three_layer_network_params():
    ...
    learning_rate = 7e-1  # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                             END OF YOUR CODE                 #
